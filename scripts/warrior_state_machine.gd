...
func map(value: float, min1: float, max1: float, min2: float, max2: float, clamp: bool = false) -> float:
...
func _physics_processALL(delta: float) -> void:	
	...
	if not jumping and not dash_timer > 0:
	...
	if dash_timer == 0 and bounce_timer == 0:
...
func _on_hit_box_body_entered(_body: Node2D) -> void:
	...
	if _body.is_in_group("Player") and _body != self:
		if abs(abs(global_position.y) - abs(_body.global_position.y))  >= 4.0 and (
			...
			_body.bounce_timer = BOUNCE_TIME
